...
## Web UI 部署
Web UI 位于 `packages/web/`，为 Next.js 应用，部署前确保服务器已安装 `yt-dlp` 与 `ffmpeg`。
...
### 队列与存储
- 任务队列使用 SQLite，默认数据库路径为 `packages/web/data/x-downloader.db`。
- 部署时请确保 `packages/web/data` 可写并持久化。
